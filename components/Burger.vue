...
const checkScrolled = () => {
  ...
  isScrolled.value = scrolledDistance >= window.innerHeight * 0.4;
}
...
<template>
  <header
    :class="['z-[900] header fixed top-8 right-8 flex justify-center items-center rounded-full w-16 h-16 sm:w-18 sm:h-18 transition-all bounce-transition', isScrolled || isActive ? 'flex scale-1' : 'flex sm:scale-0 sm:invisible', isActive ? 'active bg-black' : '']">
    <div @mousemove="(e) => useMagnet(e, 1)" @mouseout="(e) => useResetMagnet(e)" @click="isActive = !isActive"
      :class="['z-10 w-16 h-16 sm:w-18 sm:h-18 flex justify-center items-center cursor-pointer rounded-full active border', isActive ? 'bg-black border-black' : 'bg-white']">
      <div @mousemove="(e) => useMagnet(e, 0.4)" @mouseout="(e) => useResetMagnet(e)"
        class="w-16 h-16 sm:w-18 sm:h-18 group flex justify-center items-center rounded-full">
        <div
          :class="['w-6 h-[12px] sm:w-8 sm:h-[18px] flex flex-col items-end justify-between burger-menu-container', isActive && 'active-burger items-center']">
          <div
            :class="['transition-all duration-300 group-hover:w-full rounded-full w-full h-[2px] burger-menu-item', isActive ? 'bg-white' : 'bg-black']">
          </div>
          <div
            :class="['transition-all duration-300 group-hover:w-full rounded-full w-1/2 h-[2px] burger-menu-item', isActive ? 'bg-white' : 'bg-black']">
          </div>
          <div
            :class="['transition-all duration-300 group-hover:w-full rounded-full w-2/3 h-[2px] burger-menu-item', isActive ? 'bg-transparent' : 'bg-black']">
          </div>
        </div>
      </div>
    </div>
    <nav
      ...
      <ul class="flex flex-col justify-center items-center">
        <li>
          <NuxtLink @click="isActive = !isActive" to="/" :class="['text-center !text-4xl text-white uppercase']">Home
          </NuxtLink>
        </li>
        <li>
          <NuxtLink @click="isActive = !isActive" to="/projects" :class="['text-center !text-4xl text-white uppercase']">
            Projects
          </NuxtLink>
        </li>
        <li>
          <NuxtLink @click="isActive = !isActive" to="/about" :class="['text-center !text-4xl text-white uppercase']">
            About
          </NuxtLink>
        </li>
      </ul>
      <ul class="flex absolute bottom-24 justify-start items-center gap-4">
        <li>
          <a href="https://www.instagram.com/" target="_blank" class="hover:opacity-70">
        ...
        <li>
          <a href="https://www.linkedin.com/" target="_blank" class="hover:opacity-70">
            <svg class='h-10 fill-white' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
        ...
        <li>
          <a href="https://www.behance.net/" target="_blank" class="hover:opacity-70">
            <svg class='h-10 fill-white' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
